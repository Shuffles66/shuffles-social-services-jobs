...
- **Last updated:** v1.10.15 (2026-06-09) — added the on-platform / privacy-and-dignity / no-"post-piracy" core objective (hero guardrail + participant-privacy rule; mirrors `SAFETY-GUARDRAILS.md`, `EXEC-SUMMARY.md`, `MARKETING-MASTER.md`).
...
- **On-platform by design (a core objective).** Listings, requests and profiles are meant to live and be acted on inside the platform, not copied out onto public social media. This maintains privacy, increases dignity, and reduces the well-meaning copying or resharing of posts by the public that can expose people and become an unhealthy distraction. Anonymous advertising, per-field masking, pseudonymity and the internal relay all serve this objective.
...
- **On-platform by design (core objective).** The combined effect of pseudonymity, logged-in-only participant boards, anonymous advertising, per-field masking, the internal relay and `noindex` is that listings are meant to be acted on **inside the platform**, not copied out onto public social media. This deliberately **maintains privacy**, **increases dignity**, and **reduces the well-meaning copying or resharing of posts** by the public, which can re-expose people, defeat the safeguards above, and become an unhealthy distraction. Surfaced to members in the hero "Safety, built in" strip (`Shuffles_SSJ_Display::safety_guardrails`) and the Business Logic tab.
- Worker profiles honour their `visibility` flag (`public` / `logged_in` / `hidden`) at **every** layer, never just one: the directory **query** (`Shuffles_SSJ_Query::worker_args`), the **page link** (`Shuffles_SSJ_SEO::guard_private_singles` on `template_redirect` — a non-public profile can't be opened by its direct URL by anyone who couldn't see it in the directory; the owner/admins can always preview; guests on a `logged_in` profile are sent to log in, everyone else gets a 404), search-engine **indexing** (`noindex`), and the **sitemap** (`wp_sitemaps_posts_query_args` excludes non-public workers). Profiles never expose PII. A **hidden org** (`org_hidden`) is enforced the same way at the page link + sitemap — only its own team and site admins can open it. *Principle: never rely on hiding a listing from the directory alone — guessing the URL must not bypass it.*

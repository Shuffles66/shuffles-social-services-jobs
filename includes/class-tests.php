		...
		$s = array(
			...
			array(
				...
				'cases' => array(
					...
					array( 'id' => 'app-status', 'do' => __( 'As the advertiser of a “Full pipeline” job, move an applicant New → Shortlisted → Interview → Offer → Hired in My listings, then open the applicant’s “Status history”. Compare with a job set to “Simple”.', 'shuffles-social-services-jobs' ), 'expect' => __( 'Full-pipeline jobs offer all stages (incl. Hired/Declined) and record a Status history; Simple jobs show only a minimal set. The applicant is emailed on each change.', 'shuffles-social-services-jobs' ) ),
					array( 'id' => 'app-withdraw', 'do' => __( 'As the candidate, open My dashboard → My applications and click Withdraw on an active application.', 'shuffles-social-services-jobs' ), 'expect' => __( 'The application is marked Withdrawn (and recorded in its history). Already-hired or already-withdrawn applications show no Withdraw button.', 'shuffles-social-services-jobs' ) ),
				),
			),
			array(
				...
				'cases' => array(
					...
					array( 'id' => 'resume-store', 'do' => __( 'In My dashboard → “My résumés” (or [sssj_resumes]), upload a PDF/Word résumé, name it, tick “make default”; upload a second; set the other as default; remove one. Open a résumé’s file link while logged out / as another non-admin member.', 'shuffles-social-services-jobs' ), 'expect' => __( 'Multiple résumés store and list with a Default badge; making one default unsets the other; removing works; the file link only opens for the owner or an admin (others get “Not allowed”). Only PDF/DOC/DOCX/RTF/ODT up to 8 MB are accepted, up to 5 résumés.', 'shuffles-social-services-jobs' ) ),
					array( 'id' => 'asset-resume', 'do' => __( 'Open My dashboard → “Create an asset” (or the [sssj_create_asset] page) as a member with a worker profile. Switch between Résumé, Service flyer and Social post with the type selector (advertisers also get Job flyer, which lists your jobs to pick one); edit the headline and introduction and watch the preview update; read the readability check; then click Download PDF, Save image and Copy caption. Try it with no worker profile too.', 'shuffles-social-services-jobs' ), 'expect' => __( 'Each type previews from your profile with your LOCATION in the header (résumé and flyer are one page; the social post is a square). Editing the headline/intro updates it live; the readability check flags anything missing (location, services, blurb length, a verified check). Download PDF opens the print dialog showing only the résumé; Save image downloads a PNG; Copy caption copies social text. With no profile, it prompts you to create one first.', 'shuffles-social-services-jobs' ) ),
					array( 'id' => 'asset-render', 'do' => __( 'In Settings → Asset Rendering, leave it on “Browser” and open the résumé builder. Then set it to “Server”, enter your render service URL (e.g. a self-hosted Gotenberg at http://127.0.0.1:3000), tick self-hosted, save, click “Test connection”, and reopen the résumé builder.', 'shuffles-social-services-jobs' ), 'expect' => __( 'On “Browser” the builder shows a single “Download PDF” (browser print). On “Server” it shows “Print-quality PDF” (server) plus “Quick PDF (browser)”. “Test connection” reports success once the service is reachable; “Print-quality PDF” downloads a crisp server-rendered PDF of the résumé/flyer/job flyer. If the service is unreachable it shows an error and the browser path still works. Nothing breaks when no service is configured.', 'shuffles-social-services-jobs' ) ),
					...
					array( 'id' => 'worker-vis', 'do' => __( 'Set a worker profile to “Logged-in members”, then (a) view the directory as a guest and (b) open that profile’s direct web link as a guest.', 'shuffles-social-services-jobs' ), 'expect' => __( 'It is hidden from guests in the directory; opening its direct link as a guest sends you to log in (it does not render), and logged-in members can view it. The owner and admins can always preview their own page.', 'shuffles-social-services-jobs' ) ),
					...
					array( 'id' => 'media-guidance', 'do' => __( 'Open any form with a photo or logo upload (worker profile, organisation, post a job).', 'shuffles-social-services-jobs' ), 'expect' => __( 'Each image field shows an “Image guide” with preferred dimensions and what makes good, valuable, consented content (no participant identification).', 'shuffles-social-services-jobs' ) ),
					array( 'id' => 'media-video', 'do' => __( 'Paste a YouTube or Vimeo link into the “video” field on a worker, organisation or job, save, then open that single page. Also try pasting a non-YouTube/Vimeo URL.', 'shuffles-social-services-jobs' ), 'expect' => __( 'A responsive video embed (privacy-friendly) appears under the listing. A link that is not YouTube or Vimeo is rejected and nothing is embedded.', 'shuffles-social-services-jobs' ) ),
			...
			array(
				...
				'cases' => array(
					...
					array( 'id' => 'need-lifecycle', 'do' => __( 'Set a request’s close date in the past (or to within the reminder window) and run the daily cron; then open My dashboard and click “Reopen”.', 'shuffles-social-services-jobs' ), 'expect' => __( 'Within the window the owner gets a “closing soon” email; past the date the request auto-closes (moves to draft, drops off the board) and the owner gets a “now closed” email; “Reopen” republishes it with a fresh close date and resumes reminders.', 'shuffles-social-services-jobs' ) ),
				),
			),
			array(
				...
				'cases' => array(
					...
					array( 'id' => 'org-hidden', 'do' => __( 'Tick “Do not display” on an org profile, then check the Organisations directory, the page source, and open the org’s direct web link as a non-member.', 'shuffles-social-services-jobs' ), 'expect' => __( 'The org drops off the directory, is excluded from the sitemap, and its page is noindex with no Organization structured data. Opening its direct link as a non-member returns “not found”; only its own team and site admins can view it.', 'shuffles-social-services-jobs' ) ),
			...
			array(
				...
				'cases' => array(
					...
					array( 'id' => 'policies-show', 'do' => __( 'Put [sssj_policies] on a page (or open Settings → Policies) and click each policy header.', 'shuffles-social-services-jobs' ), 'expect' => __( 'Eleven plain-English policies show (Complaints, Privacy, NDIS Code of Conduct, Incident Management, Safeguarding, Terms, Worker Screening, Data Retention, Cookies, Inclusion, Advertising & media); the first is open; each has a summary + key points; the footer lists NDIS/OAIC/AHRC + interpreter contacts.', 'shuffles-social-services-jobs' ) ),
					...
					array( 'id' => 'policy-advertising', 'do' => __( 'Open the “Advertising & media production” policy (in [sssj_policies] or its /docs template).', 'shuffles-social-services-jobs' ), 'expect' => __( 'It states we prefer not to use children (with safeguards if one must appear), that anyone in commercial media is paid fairly, and that written informed consent (including any agreed remuneration) is obtained, with truthful, inclusive content that never identifies a participant.', 'shuffles-social-services-jobs' ) ),
					array( 'id' => 'marketing-page', 'do' => __( 'Open Settings → Marketing, create the Marketing page, and view it (or put [sssj_marketing] on a page).', 'shuffles-social-services-jobs' ), 'expect' => __( 'The marketing master renders as a readable page (positioning, business logic, functional spec, audience analysis), generated from docs/MARKETING-MASTER.md, naming no third-party tools.', 'shuffles-social-services-jobs' ) ),
				),
			),
			array(
				...
				'cases' => array(
					...
					array( 'id' => 'testi-moderate', 'do' => __( 'In Settings → Testimonials, Approve a pending testimonial (try Reject and Delete on others). Then turn off “Enable testimonials” and reload a profile.', 'shuffles-social-services-jobs' ), 'expect' => __( 'Approve makes it available to feature; Reject/Delete keep it off the profile (Reject also un-features it). With testimonials disabled, the “What people say” section disappears everywhere and no new ones can be left; existing ones are kept.', 'shuffles-social-services-jobs' ) ),
				),
			),
			array(
				...
				'cases' => array(
					...
					array( 'id' => 'dash-earn-support', 'do' => __( 'Open [sssj_dashboard] with the referral program available (and, separately, with the support portal available). Look at the tab row.', 'shuffles-social-services-jobs' ), 'expect' => __( 'An “Earn” tab appears when referrals are available (showing the referral promo with the PayPal note, plus the referral dashboard) and a “Support” tab appears when a support portal is available (showing it inline). Neither tab appears when its capability is absent, and nothing breaks.', 'shuffles-social-services-jobs' ) ),

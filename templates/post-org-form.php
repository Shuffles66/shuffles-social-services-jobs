		...
		<?php else : ?>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="sssj-stack" enctype="multipart/form-data" data-sssj-busy="<?php esc_attr_e( 'Saving and checking your details (NDIS register, ABN and location)…', 'shuffles-social-services-jobs' ); ?>">
				...
				<div class="sssj-row">
					...
					<div class="sssj-field"><label><?php esc_html_e( 'Organisation email domain (lets staff attach)', 'shuffles-social-services-jobs' ); ?></label><input class="sssj-input" type="text" name="org_email_domain" value="<?php echo esc_attr( $gm( 'org_email_domain' ) ); ?>" placeholder="e.g. acme.org.au" /><p class="description"><?php esc_html_e( 'The part after the @ in your work email. Workers whose account email is on this domain can request to attach to your organisation, and you approve them in your Team tab. Leave blank to switch self-attach off.', 'shuffles-social-services-jobs' ); ?></p></div>
